...
const CartItem = ({ item, setCart, setRender, render }) => {
    const [count, setCount] = useState(item.quantity);

    const updateCart = (operation) => {
        fetch(import.meta.env.VITE_API_URL + `/update-cart-qty?userId=${import.meta.env.VITE_USER}&id=${item._id}&ope=${operation}`, {
            method: "PUT",
    ...
    const deleteCart = (id) => {
        fetch(import.meta.env.VITE_API_URL + `/delete-cart/${import.meta.env.VITE_USER}/${id}`,{
            method: "delete",
            headers: {"Content-Type": "application/json"}
        })
        ...
        .catch(err => toast.error("Error while deleting Cart"));
    }
            ...
            <div className="row pb-3">
                ...
                <div className="col-6 my-auto pt-2">
                    ...
                    <div className="w-75 mx-auto">
                        {
                            (count == 1) ? <i className="fa-solid fa-trash-can text-dark fs-4 me-2 cursor" 
                                onClick={() => {
                                    deleteCart(item._id);
                                }}
                            />
                                : 
                            <i className="fa-solid fa-square-minus text-danger fs-4 me-2 cursor"
                                onClick={() => {
                                    if (count > 1) {
                        ...
                        <i
                            className="fa-solid fa-square-plus text-success fs-4 ms-2 cursor"
                            onClick={() => {
